...
from ObjectCenter import ObjectCenter
from PID import PID
...
def set_servos(pan, tlt):
	...
	while True:
		...
		panAngle = pan.value #-1 * pan.value
		tltAngle = tlt.value #-1 * tlt.value

		# if the pan angle is within the range, pan
		client.sendData(panAngle,tltAngle)
		#print('Pan %f! Titl %f.'%(panAngle,tltAngle))
...
if __name__ == "__main__":
	...
	with Manager() as manager:
		...
		panI = manager.Value("f", 0.042)
		panD = manager.Value("f", 0)

		# set PID values for tilting
		tiltP = manager.Value("f", 0.05)
		tiltI = manager.Value("f", 0.042)
